...
function FilterAppointedPatientData(Data) {
    ...
    for (Count = 0; Count < DataLength; Count++) {
        ...
        _ArrayAppointedPatientData.push({
            ...
            "Action": '<button class="btn btn-info btn-icon w-25 custom-btn" type="button" onclick="LoadVitals(' + Data[Count].Id + ')">' + '<span class="ul-btn__icon"><i class="i-Pen-2"></i></span>' + '</button>' + '<button class="btn btn-info btn-icon w-25 custom-btn mx-2" type="button" onclick="UploadFile(' + Data[Count].Id + ')">' + '<span class="ul-btn__icon"><i class="i-Upload"></i></span>' + '</button>' + '<button class="btn btn-info btn-icon w-25 custom-btn" type="button" onclick="MedicalBillDisplay(' + Data[Count].Id + ')">' + '<span class="ul-btn__icon"><i class="i-Billing"></i></span>' + '</button>'
        });
...
function medicalBillInputsValidate(PatientId) {
    ...
    const DateTime = new Date().getFullYear() + '-' + ("0" + new Date().getMonth()).slice(-2) + '-' + ("0" + new Date().getDate()).slice(-2) + ' ' + ("0" + new Date().getHours()).slice(-2) + '-' + ("0" + new Date().getMinutes()).slice(-2) + '-' + ("0" + new Date().getSeconds()).slice(-2);
    ...
    const JsonObjectToSave = {
        'Patient': Patient, 'Bill': MedicalBill,
    }
